...
| 14 | **F-034** (subir TIR) — sola | Comparte el contrato de la fila de propuesta con F-033 (mandato del plan) | **completada 10/08/2026** — salió 100 % frontend: `tipo == "mejora_rendimiento"` ya era el complemento exacto de la banda ±0,5pp de F-033, así que los dos modos particionan las candidatas de F-032 sin tocar el backend. No se unificó el tipo de salida (F-033 filtra, F-034 declara) pero sí el vocabulario por eje en `lib/rotaciones/ejes.ts`, que es lo que F-036 va a consumir de ambos. Verificar contra el backend real destapó un 500 preexistente en `/rotaciones` (`Decimal` de `public.puntas` sumado con el arancel `float`) que dejaba a F-033 y F-035 sin funcionar end-to-end |
| 15 | **F-036** (aceptación por rotación) — sola | Consume F-033 + F-034 + F-035 | **completada 10/08/2026** — 100 % frontend, los tres endpoints (`/rotaciones`, `/concentracion`, `/calendario/cartera`) ya existían. `PlanRotacionProvider` guarda sólo la pila de aceptadas y las descartadas; todo lo demás se deriva, así que deshacer (LIFO puro, sólo la última) es exacto por construcción. El efecto de calendario por fila compara mes a mes y moneda por moneda, sin sumar entre monedas. Verificado en vivo: aceptar recalcula sobre la cartera resultante de verdad (nuevo origen en las propuestas restantes), deshacer vuelve bit por bit, lo descartado y la inversa de lo aceptado no se re-proponen. `Cordillera` salió de `DiagnosticoCartera.tsx` a componente compartido |
| 16 | **F-037** (comparación original/propuesta) — sola | Consume F-036 | **completada 10/08/2026** — también 100 % frontend, sin endpoint nuevo. `ComparacionCarteras` mide las dos columnas con las mismas funciones puras (`rendimientosPorNaturaleza`, `vectorDeRiesgo`, `calcularRentaAnualPorMoneda`) sobre las posiciones de cada lado, así que "misma vara" es consecuencia de la implementación. El costo total se lleva a plata sobre el monto encadenado (nunca se suman `total_pct` de bases distintas) y declara lo no verificable/no convertible con el par nombrado. El diff de calendario de F-036 se extrajo a un núcleo compartido (`diffMesAMes`) para una variante nueva, `diffCalendarioCarteras`, que marca en la cordillera propuesta los meses que cambian de cobertura, moneda por moneda (`Cordillera` ganó la prop `marcas`). El delta de los seis ejes se muestra sin color de mejora/empeora — a diferencia de renta y rendimiento, ningún eje tiene una dirección universal de "mejor" (regla 7). `PlanDeRotacion` se podó a la lista de aceptadas y Deshacer; el calendario y los ejes de la propuesta pasaron enteros a la comparación. Verificado en vivo contra el backend real: aceptar AL30D→BGC4D mostró las dos columnas con delta explícito, los meses marcados (▲/▼) y el costo total (US$ 152,87) junto a la mejora de renta y de TIR; deshacer hizo desaparecer la comparación entera y devolvió las tres propuestas originales |
| 17 | **F-041** (guardar/reabrir carteras) — sola | Consume F-037 | **completada 10/08/2026** — el snapshot (jsonb, unión discriminada por origen `cargada`/`armador`, `strictObject` en todos los niveles propios) se persiste directo del frontend a Supabase por PostgREST con RLS (`carteras.user_id DEFAULT auth.uid()`), sin endpoint nuevo: se saldó así la deuda de la Tanda 1 sobre el pool del backend saltando RLS, sin tocarlo. GWT-1 es consecuencia de la implementación —`VistaCongelada` sólo renderiza el snapshot, cero requests de mercado hasta tocar "Revaluar a hoy", que recién ahí monta los mismos servicios que el resto de la app (`useCarteraCargadaValuada` para origen cargada, universo+TC de hoy para armador) y compara sin promediar monedas. "Abrir en el armador" rehidrata sólo el mandato, no la foto valuada. Verificado en vivo contra el proyecto real de Supabase: guardar, reabrir, revaluar (delta por posición y total con TC y fecha de cada punta), reabrir en el armador (mismos % pedidos) y borrar, para los dos orígenes; RLS verificado por SQL con dos `sub` simulados (B ve 0 filas de A, insert suplantando a A rechazado); PII verificado inspeccionando las claves reales del snapshot guardado |
| 18 | **F-042** (export Excel/PDF) — sola | Consume F-041 | **completada 10/08/2026** — 100% frontend: `modeloDesdeSnapshot()` es la única puerta de entrada, sea el snapshot guardado o armado en vivo, y `write-excel-file`/`jspdf`+`jspdf-autotable` se cargan con `import()` dinámico sólo al hacer click (confirmado en `npm run build` que quedan en chunks propios). El snapshot de F-041 ganó un bloque `mercado` opcional (naturaleza, lámina, vector de seis ejes, calendario, fuente del dato) — dentro de `version: 1`, no una versión nueva, así que una fila guardada antes de esta feature sigue exportando con los faltantes declarados por nombre. Verificado en vivo contra el backend real y Supabase: cartera armada → guardada → reabierta → exportada a Excel y PDF, los dos archivos leídos con `openpyxl`/`pdfplumber` para confirmar contenido — GWT-1 (rendimientos abiertos por naturaleza, ninguna celda los combina), GWT-2 (posiciones sin lámina declaradas con su %) y GWT-3 (pie con hora de precios y demora de la fuente, repetido en las dos páginas del PDF de la prueba) los tres con datos reales, no sólo con fixtures. De paso cerró la deuda de diseño de `ColumnaKpis` (flujo mes por mes + botón "Descargar propuesta", design-system.md:118-123). Cierra el Ciclo 4 y el camino crítico completo del plan |
