 ...
 * `CarteraEditable` (precio, moneda de cotización, emisor por ticker) hay que traer el universo
 * entero una vez y quedarse con lo que hace falta. Es el mismo bucle de cursor que
 * `features/monitor/hooks/useUniversoSegmento.ts` (F-038), portado acá en vez de importado —
 * `features/monitor/**` está prohibido para esta feature, y además ese hook siempre filtra por
 * `?segmento=`, mientras que acá se pide el universo sin filtrar.
 */

import { apiFetch } from '@/lib/api/client'
import { esquemaPagina } from '@/lib/api/schemas'

import { esquemaEspecie, type Especie } from './schema'
