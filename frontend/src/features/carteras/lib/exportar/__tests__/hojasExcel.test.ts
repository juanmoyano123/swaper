...
describe('GWT-1: rendimientos abiertos, número crudo con formato porcentual', () => {
  ...
  it('las cuatro naturalezas aparecen en filas separadas, ninguna celda las combina', () => {
    const filas = hoja(modeloBase(), 'Rendimientos')
    // encabezado + 4 naturalezas + fila vacía + plazo promedio = 7
    const nombresDeNaturaleza = filas.slice(1, 5).map((f) => f[0].valor)
