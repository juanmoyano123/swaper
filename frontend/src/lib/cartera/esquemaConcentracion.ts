 ...
 *
 * Movido acá en la Tanda 11 desde `features/armador/lib/schemaConcentracion.ts`, para que el
 * diagnóstico de cartera (F-030) mida sus topes con el mismo contrato que el armador en vez de
 * duplicarlo (riesgo R12 de `plan.md:2774`).
 *
 ...
 * redefinen acá en vez de importarse de `esquemaCalendario.ts` por la misma razón que allá.
 */
