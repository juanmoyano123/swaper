...
normalización. Acá se prueba lo que sólo se ve con los cinco endpoints juntos: que el conteo por
endpoint quede en el snapshot, que uno caído no corte a los otros cuatro, y que la demora declarada
sea un atributo del snapshot.
"""
...
async def test_la_ingesta_registra_el_conteo_de_filas_de_los_cinco_endpoints(
    settings_de_prueba,
) -> None:
    """Dos endpoints como lista plana, dos como objeto paginado multi-página, uno de una página."""
    with respx.mock:
...
async def test_un_endpoint_caido_no_impide_ingerir_los_otros_cuatro(settings_de_prueba) -> None:
    with respx.mock:
    ...
    assert len(resultado.especies) == 3  # negociable-obligations + cedears + general-equity
...
async def test_un_500_persistente_agota_los_reintentos_y_no_corta_la_corrida(
    ...
    # Los otros cuatro se ingirieron igual.
    assert len(resultado.especies) == 3
    assert len(resultado.indices) == 1
...
async def test_el_snapshot_declara_la_demora_de_veinte_minutos(settings_de_prueba) -> None:
    with respx.mock:
        ...
        for endpoint in ("public-bonds", "general-equity", "index-price"):
            respx.post(f"{BASE_URL}/{endpoint}").mock(
